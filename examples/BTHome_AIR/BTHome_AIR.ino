...
// not scheduled today
int whenIsNextSheduleInSeconds(const DateTime& now, const Schedule& s) {
  ...
  if (nowMin >= startMin && nowMin < endMin) {
    ...
    return (startMin - nowMin) * 60;  // minutes until next start
  } else {
...
// bthome.addMeasurement(ID_CURRENT, to_battery_current);  // 3
  ...
  for (int i = 0; i < programLength; i++) {
    ...
    int when = whenIsNextSheduleInSeconds(now, programs[i]);
    //if zero then we should run now
    if (when == 0) {
      ...
      //-1 means not today or already happened
      if (when != -1) {
        if (soonest == 0) {
          soonest = when;
        }
  ...
  if (soonest != 0) {
    bthome.addMeasurement(ID_DURATION, (uint64_t)soonest);  // 3
  }
  if (relayStatus != shouldBeOn) {
    digitalWrite(RELAY_PIN, shouldBeOn);
  }
  bthome.addMeasurement_state(STATE_POWER_ON, digitalRead(RELAY_PIN));  // 1
